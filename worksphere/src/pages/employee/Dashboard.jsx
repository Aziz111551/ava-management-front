  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '28px' }}>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '24px', fontWeight: '800', color: 'var(--text)', letterSpacing: '-0.5px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '28px' }}>
        <FeatureCard title="My Projects" description="Manage and track your active projects" gradient="var(--grad-cyan)" icon="📁" onClick={() => navigate('/employee/projets')} />
        <FeatureCard title="Trello Tasks" description="Your ongoing sprint tasks" gradient="var(--grad-blue)" icon="✅" onClick={() => navigate('/employee/taches')} />
        <FeatureCard title="Leave Request" description="Submit a new request" gradient="var(--grad-pink)" icon="🌴" onClick={() => navigate('/employee/conges')} />
        <FeatureCard title="Sick Leave" description="Declare a medical leave" gradient="var(--grad-amber)" icon="🏥" onClick={() => navigate('/employee/maladie')} />
      </div>
      ...
      {/* Projects */}
      <SectionTitle action={<Btn small variant="ghost" onClick={() => navigate('/employee/projets')}>View all →</Btn>}>
        Ongoing projects
      </SectionTitle>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '28px' }}>
        {projects.map(p => (
          <div key={p._id} style={{
            ...
            <div style={{ fontSize: '11px', color: 'var(--text3)' }}>🗓 Deadline: {new Date(p.deadline).toLocaleDateString('en-US')}</div>
          </div>
        ))}
      </div>
      ...
      <SectionTitle action={
        <div style={{ display: 'flex', gap: '8px' }}>
          <Btn small variant="ghost" onClick={() => window.open('https://trello.com', '_blank')}>Open Trello ↗</Btn>
          <Btn small onClick={() => navigate('/employee/taches')}>View all →</Btn>
        </div>
      ...
      {/* My HR Requests */}
      <SectionTitle action={<Btn small onClick={() => navigate('/employee/conges')}>New request →</Btn>}>
        My HR Requests
      </SectionTitle>
